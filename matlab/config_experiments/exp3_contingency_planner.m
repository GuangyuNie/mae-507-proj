...
% ADD ANOTHER PENALTY IF ROBOT GOES TO LEFT/RIGHT LANE AND OPPOSITE LANE
lower = [0, -7.75] - params.obs_padding;
upper = [3.65, -3.65] + params.obs_padding;
params.turnleft_lane_sd = shapeRectangleByCorners(params.g2d, lower, upper);
params.sd_obs = min(params.raw_sd_obs, params.turnleft_lane_sd);

% ADD ANOTHER PENALTY IF ROBOT GOES TO LEFT/RIGHT LANE AND OPPOSITE LANE
lower = [-3.65, 3.65] - params.obs_padding;
upper = [0, 7.75] + params.obs_padding;
params.turnright_lane_sd = shapeRectangleByCorners(params.g2d, lower, upper);
params.sd_obs = min(params.sd_obs, params.turnright_lane_sd);

% ADD ANOTHER PENALTY IF ROBOT GOES TO LEFT/RIGHT LANE AND OPPOSITE LANE
lower = [3.65, -3.65] - params.obs_padding;
upper = [7.75, 0] + params.obs_padding;
params.backward_lane_sd = shapeRectangleByCorners(params.g2d, lower, upper);
params.sd_obs = min(params.sd_obs, params.backward_lane_sd);

% ADD ANOTHER PENALTY IF ROBOT GOES TO LEFT/RIGHT LANE AND OPPOSITE LANE
lower = [-7.75, -3.65] - params.obs_padding;
upper = [-3.65, 0] + params.obs_padding;
params.opposite_lane_sd = shapeRectangleByCorners(params.g2d, lower, upper);
params.sd_obs = min(params.sd_obs, params.opposite_lane_sd);
